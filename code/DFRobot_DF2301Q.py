...
class DFRobot_DF2301Q_I2C(DFRobot_DF2301Q):
  ...
  def __init__(self, i2c_addr=DF2301Q_I2C_ADDR, bus=1):
    ...
    '''
    if smbus is None:
      raise ImportError("smbus is required for I2C mode. Use UART mode or install python3-smbus.")
    self._addr = i2c_addr
...
class DFRobot_DF2301Q_UART(DFRobot_DF2301Q):
  ...
  def __init__(self, port="/dev/serial0", timeout=0.5):
    '''!
      @brief Module UART communication init
    '''
    self.uart_cmd_ID = 0
    self._send_sequence = 0
    self._ser = serial.Serial(port, baudrate=DF2301Q_UART_BAUDRATE, bytesize=8, parity='N', stopbits=1, timeout=timeout)
  ...
  def _recv_packet(self):
    ...
    while self._ser.in_waiting:
      ...
      if not raw:
        return
      receive_char = raw[0] if isinstance(raw, (bytes, bytearray)) else ord(raw)
